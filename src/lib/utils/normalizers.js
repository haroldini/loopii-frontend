...
import { allPlatforms } from "$lib/stores/app";
...
export function normalizeSocial(social) {
	...
	if (handle) {
		// Remove accidental leading characters users might type
		if (handle.startsWith("@") || handle.startsWith("/")) {
			handle = handle.slice(1);
		}

		// Strip leading whitespace (normalizeString already trims)
		// but make sure no spaces are inside
